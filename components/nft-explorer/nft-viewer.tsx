...
const NftViewer = ({ mockNFTs }: NftViewerProps) => {
  ...
  function parseFiltersFromURL(params: URLSearchParams): FilterState {
    ...
    const marketplaces =
      params.get("marketplace")?.split(",").filter(Boolean) || [];
  ...
  function serializeFiltersToURL(filters: FilterState): string {
    ...
    if (filters.priceRange.currency && filters.priceRange.currency !== "ETH")
      params.set("currency", filters.priceRange.currency);
    if (filters.marketplaces.length > 0)
      params.set("marketplace", filters.marketplaces.join(","));
  ...
  function extractAvailableTraits(
    nfts: { traits?: Record<string, string> }[]
  ): TraitCategory[] {
    ...
    return Object.entries(traitMap).map(([name, valuesObj]) => ({
      name,
      values: Object.entries(valuesObj).map(([value, count]) => ({
        value,
        count: Number(count),
      })),
    }));
