...
export default function CollectionsPage({ activeTab, searchQuery, blockchain }: CollectionsPageProps) {
    ...
    const filteredCollections = collectionsData
        .filter((collection) => {
            ...
            const matchesBlockchain = collection.blockchain === blockchain;
        ...
        <div className="bg-black text-white min-h-screen">
            <div className="py-8">
                    ...
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="text-gray-400 border-b border-gray-700">
                                <th className="py-3 text-left pl-4 w-16">#</th>
                                <th className="py-3 text-left">COLLECTION</th>
                                <th className="py-3 text-left">FLOOR PRICE</th>
                                <th className="py-3 text-left">VOLUME</th>
                                <th className="py-3 text-left">TOP OFFER</th>
                                <th className="py-3 text-right">SALES</th>
                                <th className="py-3 text-center">FLOOR ID</th>
                                <th className="py-3 text-right pr-4">HOLDERS</th>
                            </tr>
                        </thead>
                        <tbody>
                            {visibleCollections.map((collection) => (
                                <tr key={collection.id} className="border-gray-800 hover:bg-gray-900">
                                    <td className="py-4 pl-4">{collection.id}</td>
                                    <td className="py-4">
                                        <div className="flex gap-3 items-center">
                                            <div className="relative w-10 h-10">
                                                ...
                                                <Image
                                                    ...
                                                    width={60}
                                                    height={60}
                                                    className="w-4 h-4 rounded-full bg-white absolute top-10 right-0 transform translate-x-1/2 -translate-y-1/2"
                                                />
                                    ...
                                    <td className="py-4">{collection.topOffer}</td>
                                    <td
                                        className={`py-4 text-right ${collection.change?.toString().trim().startsWith("+")
                                                ? "text-green"
                                                : collection.change?.toString().startsWith("-")
                                                    ? "text-red"
                                                    : ""
                                            }`}
                                    >
                                    ...
                                    <td className="py-4 text-center">{collection.floorId}</td>
                ...
                <div className="mt-8 text-center text-gray-400">
                    Showing {visibleCollections.length} of {totalCollections} collections
                </div>
                {hasMoreCollections && (
